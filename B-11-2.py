...
def update():
    ...
    for i in range(0, balls):
        ...
        if VB[i].y >= 200:
            pyxel.play(0, 1)
            VB[i].x = random.randint(0, 199)
            VB[i].y = 0
            angle = math.radians(random.randint(30,150))
            VB[i].x = math.cos(angle) * Ball.speed
            VB[i].y = math.sin(angle) * Ball.speed
            VB[i] = Ball()
            miss += 1
        ...
        if VB[i].y >= 195 and VP.padx-20 <= VB[i].x <= VP.padx+20:
            ...
            VB[i].y = 0
            angle =math.radians(random.randint(30,150))
            VB[i].vx = math.cos(angle) * Ball.speed
            VB[i].vy = math.sin(angle) * Ball.speed
            p += 1
            num1 = random.randint(4,10)
            num2 = random.randint(1,15)
            if p == 10 and balls < 3:
